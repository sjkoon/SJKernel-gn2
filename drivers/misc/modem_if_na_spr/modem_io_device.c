...
static int get_hdlc_size(struct io_device *iod, char *buf)
{
	...
	pr_info("[MODEM_IF] buf : %02x %02x %02x (%d)\n", *buf, *(buf + 1),
				*(buf + 2), __LINE__);
...
static int rx_hdlc_head_check(struct io_device *iod, char *buf, unsigned rest)
{
	...
	if (!hdr->start) {
		...
		pr_info("[MODEM_IF] check len : %d, rest : %d (%d)\n", len,
					rest, __LINE__);
	...
	pr_info("[MODEM_IF] check done_len : %d, rest : %d (%d)\n", done_len,
				rest, __LINE__);
	return done_len;
}
...
static int rx_iodev_skb(struct io_device *iod)
{
	...
	default:
		...
		pr_info("[MODEM_IF] wake up fmt,rfs skb\n");
		wake_up(&iod->wq);
		return 0;
	}
}
...
static long misc_ioctl(struct file *filp, unsigned int cmd, unsigned long _arg)
{
	...
	pr_info("[MODEM_IF] misc_ioctl : 0x%x\n", cmd);
	...
	case IOCTL_MODEM_FORCE_CRASH_EXIT:
		pr_info("[MODEM_IF] misc_ioctl : IOCTL_MODEM_FORCE_CRASH_EXIT\n");
	...
	case IOCTL_MODEM_STATUS:
		pr_info("[MODEM_IF] misc_ioctl : IOCTL_MODEM_STATUS\n");

		/* simulating modem reset when it crashes */
		/*
			...
			loff_t *f_pos)
{
	...
	pr_info("[MODEM_IF] copy to user : %d\n", pktsize);
	pr_info("MIF : <%s->\n", __func__);
